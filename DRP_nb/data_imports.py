...
import pandas as pd
import sys
from DRP_nb.utils import keep_overlapping
...
def read_inputs_phos_ect():
    '''Reads input data for overlap of phos, rna and prot omics data.
    ...
    ic50_df1, prot = keep_overlapping(ic50_df1, prot)
    ...
    phos_raw, ic50_df1 = keep_overlapping(phos_raw, ic50_df1)
    ...
    phospho_ls, rna = keep_overlapping(phospho_ls, rna_raw)
    phospho_ls.shape, rna.shape

    prot, rna = keep_overlapping(prot, rna)
    phospho_ls, rna = keep_overlapping(phospho_ls, rna)
    _all_cls = prot.index
...
def read_inputs_rna_prot():
    ...
    prot, rna = keep_overlapping(prot, rna_raw)
    _all_cls = prot.index
